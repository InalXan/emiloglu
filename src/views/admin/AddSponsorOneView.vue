...
const submitSponsor = async () => {
  ...
  try {
    const response = await axios.post('http://localhost:5000/admin/sponsor/one', formData, {
      headers: {
...
const fetchSponsors = async () => {
  loading.value = true
  try {
    const response = await axios.get('http://localhost:5000/admin/sponsor/one')
...
const deleteSponsor = async (id) => {
  if (confirm('Tərəfdaşı silməyə əminsiz?')) {
    try {
      await axios.delete(`http://localhost:5000/admin/sponsor/one/${id}`)
...
const updateSponsor = async () => {
  ...
  try {
    ...
    const response = await axios.put(
      `http://localhost:5000/admin/sponsor/one/${editingSponsor.value._id}`,
      formData,
